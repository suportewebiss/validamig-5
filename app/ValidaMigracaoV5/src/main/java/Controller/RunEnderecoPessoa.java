 ...
 * @author vinicius
 */
public class RunEnderecoPessoa 
{
    private ArrayList<Header> header = new ArrayList<Header>();
    private ArrayList<EnderecoPessoa> dadosAntigo = new ArrayList<EnderecoPessoa>();
    private ArrayList<EnderecoPessoa> dadosNovo = new ArrayList<EnderecoPessoa>();
    private JLabel progress;
    private String ColunaChave;
    JSONObject template;
    XSSFWorkbook oldWorkbook;
    XSSFWorkbook newWorkbook;
    public RunEnderecoPessoa(FileInputStream streamOldFile, FileInputStream streamNewFile, JSONObject template, JLabel progress)
    {
       ...
       // Inicia as WorkBooks (Excel) através do ponteiro do stream
        try {
            ...
            Logger.getLogger(RunEnderecoPessoa.class.getName()).log(Level.SEVERE, null, ex);
        }
        ...
        new Thread() {
            ...
            public void run() {
                ...
                while (rowIterator.hasNext())
                {
                    ...
                    EnderecoPessoa at = new EnderecoPessoa();
                ...
                Iterator<EnderecoPessoa> iterator = dadosAntigo.iterator();
                while (iterator.hasNext())
                {
                    // Pega o objeto com os dados da planilha antiga
                    EnderecoPessoa old = iterator.next();
                    ...
                    for (Integer index = 0; index < dadosNovo.size(); index++ )
                    {
                        ...
                        if (old.getString(ColunaChave).trim().toLowerCase().equals(dadosNovo.get(index).getString(ColunaChave).trim().toLowerCase()))
                        {
                           ...
                           // Inicamos a comparacao
                            ...
                            EnderecoPessoa neww = dadosNovo.get(index);
    ...
    private boolean comparaIgualdade(String columnName, EnderecoPessoa a, EnderecoPessoa b)  
    {
    ...
    private boolean comparaReferencia(String columnName, EnderecoPessoa a, EnderecoPessoa b)  
    {
