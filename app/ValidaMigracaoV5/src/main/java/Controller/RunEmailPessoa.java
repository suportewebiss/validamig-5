 ...
 * @author vinicius
 */
public class RunEmailPessoa 
{
    private ArrayList<Header> header = new ArrayList<Header>();
    private ArrayList<EmailPessoa> dadosAntigo = new ArrayList<EmailPessoa>();
    private ArrayList<EmailPessoa> dadosNovo = new ArrayList<EmailPessoa>();
    private JLabel progress;
    private String ColunaChave;
    JSONObject template;
    XSSFWorkbook oldWorkbook;
    XSSFWorkbook newWorkbook;
    public RunEmailPessoa(FileInputStream streamOldFile, FileInputStream streamNewFile, JSONObject template, JLabel progress)
    {
       ...
       // Inicia as WorkBooks (Excel) através do ponteiro do stream
        try {
            ...
            Logger.getLogger(RunEmailPessoa.class.getName()).log(Level.SEVERE, null, ex);
        }
        ...
        new Thread() {
            ...
            public void run() {
                ...
                while (rowIterator.hasNext())
                {
                    ...
                    EmailPessoa at = new EmailPessoa();
                ...
                Iterator<EmailPessoa> iterator = dadosAntigo.iterator();
                while (iterator.hasNext())
                {
                    // Pega o objeto com os dados da planilha antiga
                    EmailPessoa old = iterator.next();
                    ...
                    for (Integer index = 0; index < dadosNovo.size(); index++ )
                    {
                        ...
                        if (old.getString(ColunaChave).trim().toLowerCase().equals(dadosNovo.get(index).getString(ColunaChave).trim().toLowerCase()))
                        {
                           ...
                           // Inicamos a comparacao
                            ...
                            EmailPessoa neww = dadosNovo.get(index);
    ...
    private boolean comparaIgualdade(String columnName, EmailPessoa a, EmailPessoa b)  
    {
    ...
    private boolean comparaReferencia(String columnName, EmailPessoa a, EmailPessoa b)  
    {
